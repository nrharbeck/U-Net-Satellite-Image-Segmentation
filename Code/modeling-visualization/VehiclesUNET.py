...
if __name__ == '__main__':
    ...
    batch_size = 128
    nb_epoch = 4
    inputs = data_generator(X_train, y_train, batch_size, horizontal_flip=True, vertical_flip=True, swap_axis=True)
    print(inputs.x)
    filepath = "vehicle.h5"
    model.compile(optimizer=Nadam(lr=1e-3), loss=jaccard_coef_loss, metrics=['binary_crossentropy', jaccard_coef_int])
    model.load_weights('vehicle.h5') #Comment this if you have not already run the model at least once, it helps to save time in subsequent training steps.
    ...
    plt.savefig('vehicle_binary_crossentropy' +str(history.history['val_jaccard_coef_int'][-1]) +'.png')
    ...
    plt.xlabel('epoch')
    plt.legend(['train', 'test'], loc='upper left')
    plt.savefig('vehicle_loss' +str(history.history['val_jaccard_coef_int'][-1]) +'.png')
