...
if __name__ == '__main__':
    ...
    #model.load_weights('t_c.h5') #Comment this if you have not already run the model at least once, it helps to save time in subsequent training steps.
    ...
    plt.savefig('t_c_binary_crossentropy' +str(history.history['val_jaccard_coef_int'][-1]) +'.png')
    ...
    plt.xlabel('epoch')
    plt.legend(['train', 'test'], loc='upper left')
    plt.savefig('t_c_loss' +str(history.history['val_jaccard_coef_int'][-1]) +'.png')
